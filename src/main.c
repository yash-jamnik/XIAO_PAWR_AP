	...
	#define WDISC_THREAD_STACK_SIZE 8192
	...
	static atomic_t last_connect_succeeded = ATOMIC_INIT(0);
	...
	static void print_conn_cb(struct bt_conn *conn, void *data)
	{
		int *count = data;
	...
	void connected_cb(struct bt_conn *conn, uint8_t err)
	{
		...
		if (err) { 
			...
			atomic_set(&last_connect_succeeded, 0);
		...
		atomic_set(&last_connect_succeeded, 1);
	...
	void gatt_thread(void *p1, void *p2, void *p3){
		...
		int ret = k_msgq_get(&scan_result_msgq, &item, K_FOREVER);
			if (ret == 0) {
				...
				if (!atomic_get(&last_connect_succeeded)) {
	...
	void write_disconnect(void *p1, void *p2, void *p3)
	{
		...
		/* ---- STEP 1: PAST ---- */
		err = bt_le_per_adv_set_info_transfer(pawr_adv, conn, 0);
		if (err) {
	...
	// 	if (conn) {
	// 		int err = bt_le_per_adv_set_info_transfer(pawr_adv, conn, 0);
	// 		if (err) {
	// 			APP_LOG("PAST failed (err %d)\n", err);
	// 		}
	// 		bt_conn_unref(conn); /* lookup gives you a new reference, must release it */
	// 	} else {
	// 		APP_LOG("No active connection found for this address — can't send PAST\n");
	// 	}
