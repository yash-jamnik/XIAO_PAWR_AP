...
// 	.interval_min = 0x200,          // Increase interval_min to 625 ms
// 	.interval_max = 0x300,          // Increase interval_max to 937.5 ms
// 	.options = 0,
// 	.num_subevents = NUM_SUBEVENTS, // Keep subevents consistent
// 	.subevent_interval = 0x50,      // Increase subevent interval to 31.25 ms
// 	.response_slot_delay = 0x10,    // Increase response slot delay to 6.25 ms
// 	.response_slot_spacing = 0x80,  // Increase response slot spacing to 50 ms
// 	.num_response_slots = NUM_RSP_SLOTS, // Keep response slots consistent
// };
...
// // static void log_synced_devices_state(void);
...
// static char current_command[CMD_BUF_SIZE] = "[+]join,9999"; // Default command
...
// 	uint8_t subevent;
// 	uint8_t response_slot;
// 	char address[20]; // Store device address as string
// 	int64_t last_update_time; // Timestamp of the last update
// };

// static struct synced_device synced_devices[MAX_SYNCS];
// #define SLOT_TIMEOUT_MS 10000  // 10 seconds
// // Helper: Find or assign slot for device by address
// static bool is_slot_responsive(int slot_index)
// {
//     // Example: Check if the slot has received data recently
//     uint32_t time_diff = k_uptime_get() - synced_devices[slot_index].last_update_time;
//     if (time_diff > SLOT_TIMEOUT_MS)
//     {
//         printk("Slot %d is unresponsive (time since last update: %u ms)\n", slot_index, time_diff);
//         return false; // Slot is not responsive
//     }
//     return true;
// }
...
//     synced_devices[slot_index].subevent = -1; // Use -1 to indicate an unused slot
//     synced_devices[slot_index].response_slot = -1; // Use -1 to indicate an unused slot
//     memset(synced_devices[slot_index].address, 0, sizeof(synced_devices[slot_index].address));
//     synced_devices[slot_index].last_update_time = 0; // Reset timestamp
//     printk("Cleared slot %d\n", slot_index);
//     // log_synced_devices_state(); // Log state after clearing
// }

// static int find_or_assign_slot(const char *address, uint8_t *subevent, uint8_t *response_slot)
// {
//     // Search all slots for a matching address
//     for (int i = 0; i < MAX_SYNCS; i++)
//     {
//         if (synced_devices[i].active && strcmp(synced_devices[i].address, address) == 0)
//         {
//             // Check if the slot is responding with new data
//             if (is_slot_responsive(i)) // Implement this function to check slot responsiveness
//             {
...
//             else
//             {
//                 // Mark the slot as inactive if it is not responsive
...
//             synced_devices[i].last_update_time = k_uptime_get(); // Set current timestamp
...
//     // log_synced_devices_state(); // Log state if no slot is available
...
// // UART polling function for reading commands
...
// // Function to send command data to all synced devices
...
// 		return 0; // Don't return error, just update the command
// 	}
...
// 		// Copy command to buffer (ensure we don't exceed buffer size)
// 		size_t copy_len = MIN(cmd_len, PACKET_SIZE - 1);
// 		memcpy(buf->data, command, copy_len);
// 		buf->data[copy_len] = '\0'; // Null terminate
// 		buf->len = copy_len + 1;
...
// 	// Force immediate update of PAwR data
// 	err = bt_le_per_adv_set_subevent_data(pawr_adv, NUM_SUBEVENTS, subevent_data_params);
// 	if (err)
// 	{
// 		printk("Failed to set PAwR command data immediately (err %d)\n", err);
// 		return err;
// 	}
...
// 	// Check if it's a join command format: [+]join,xxxx or just join,xxxx
...
// 		// Update the global command immediately
// 		strncpy(current_command, cmd, CMD_BUF_SIZE - 1);
// 		current_command[CMD_BUF_SIZE - 1] = '\0';
// 		printk("Global command updated to: '%s'\n", current_command);

// 		// Send the command to all synced devices
...
// 		else
// 		{
// 			printk("SUCCESS: Join command sent to all synced devices\n");
// 			printk("Waiting for device responses...\n");
// 		}
// 	}
// 	else if (strncmp(cmd, "join,", 5) == 0)
// 	{
// 		// Alternative format: join,xxxx (automatically add [+] prefix)
...
// 		// Update the global command immediately
// 		strncpy(current_command, full_cmd, CMD_BUF_SIZE - 1);
// 		current_command[CMD_BUF_SIZE - 1] = '\0';
// 		printk("Global command updated to: '%s'\n", current_command);

// 		display_synced_devices_status();

// 		// Send the formatted command to all synced devices
...
// 	{
// 		// Test command - sends [+]join,9999
// 		printk("Sending test join command: [+]join,9999\n");
// 		strncpy(current_command, "[+]join,9999", CMD_BUF_SIZE - 1);
// 		current_command[CMD_BUF_SIZE - 1] = '\0';
// 		printk("Global command updated to: '%s'\n", current_command);
...
// 	{
// 		// Status command to show current device list
// 		display_synced_devices_status();
// 		printk("Current active command: '%s'\n", current_command);
// 	}
// 	else if (strcmp(cmd, "refresh") == 0)
// 	{
// 		// Force refresh of current command
...
// 		// Help command
// 		printk("\nAvailable commands:\n");
// 		printk("  <number>         - Set join command (e.g., '1234' sets [+]join,1234)\n");
// 		printk("  [+]join,<param>  - Set join command to be sent to all synced devices\n");
// 		printk("  join,<param>     - Set join command (auto-format with [+] prefix)\n");
// 		printk("  test             - Set test join command [+]join,9999\n");
// 		printk("  status           - Show synced devices and current command\n");
// 		printk("  refresh          - Force refresh current command to devices\n");
// 		printk("  help             - Show this help message\n");
// 		printk("Note: Commands are updated immediately and sent via PAwR\n");
// 		printk("Examples:\n");
// 		printk("  1234             - Set join with parameter 1234 (easiest)\n");
// 		printk("  [+]join,1234     - Set join with parameter 1234 (full format)\n");
// 		printk("  join,9999        - Set join with parameter 9999 (medium format)\n");
// 		printk("  test             - Set test command with parameter 9999\n");
// 		printk("Current command: '%s'\n\n", current_command);
// 	}
// 	else if (strlen(cmd) > 0)
// 	{
// 		// Check if it's just a number (shortcut for join,xxxx)
// 		bool is_number = true;
...
// 			snprintf(full_cmd, sizeof(full_cmd), "[+]join,%d", join_param);
// 			printk("Numeric shortcut detected - sending join command with parameter: %d\n", join_param);
// 			printk("Formatted command: %s\n", full_cmd);
...
// 			printk("Quick usage: Type a number (e.g., '1234') to send [+]join,1234\n");
// 		}
// 	}
...
// 	static int auto_test_counter = 0;
...
// 	printk("  1234             - Set join command with parameter 1234 (number shortcut)\n");
// 	printk("  join,1234        - Set join command with parameter 1234\n");
// 	printk("  test             - Set test join command [+]join,9999\n");
// 	printk("  status           - Show synced devices and current command\n");
...
// 	size_t cmd_len = strlen(current_command);

// 	// printk("PAwR: Device requesting data - sending current command: '%s'\n", current_command);
...
// 		// Copy current command to buffer (ensure we don't exceed buffer size)
// 		size_t copy_len = MIN(cmd_len, PACKET_SIZE - 1);
// 		memcpy(buf->data, current_command, copy_len);
// 		buf->data[copy_len] = '\0'; // Null terminate
// 		buf->len = copy_len + 1;

// 		subevent_data_params[i].subevent = (request->start + i) % per_adv_params.num_subevents;
// 		subevent_data_params[i].response_slot_start = 0;
// 		subevent_data_params[i].response_slot_count = NUM_RSP_SLOTS;
// 		subevent_data_params[i].data = buf;
// 	}
...
// 	else
// 	{
// 		// printk("PAwR: Successfully set command '%s' for transmission\n", current_command);
// 	}
// }
...
// 		// Try to print as string first if it looks like text
// 		bool is_text = true;
// 		for (size_t i = 0; i < buf->len && i < 64; i++)
// 		{
// 			if (buf->data[i] < 0x20 && buf->data[i] != 0x00 && buf->data[i] != '\n' && buf->data[i] != '\r')
// 			{
// 				is_text = false;
// 				break;
// 			}
...
// 			// Parse address if response starts with "devid," (from receiver)
// 			if (buf->len > 6 && strncmp((char *)buf->data, "devid,", 6) == 0)
// 			{
// 				// Copy address part
// 				int addr_len = buf->len - 6;
// 				if (addr_len > 0 && addr_len < (int)sizeof(dev_addr))
// 				{
// 					memcpy(dev_addr, &buf->data[6], addr_len);
// 					dev_addr[addr_len] = '\0';
// 					printk("Parsed device address from response: %s\n", dev_addr);

// 					// Find or assign slot for this address
// 					uint8_t subevent, response_slot;
// 					int idx = find_or_assign_slot(dev_addr, &subevent, &response_slot);
// 					if (idx >= 0)
// 					{
// 						printk("Device slot: %d, subevent: %d, response_slot: %d\n", idx, subevent, response_slot);
// 					}
// 					else
// 					{
...
// 		// Also print raw hex data for debugging
// 		printk("Response hex: ");
// 		for (size_t i = 0; i < MIN(buf->len, 32); i++)
// 		{
...
// 			for (int i = 0; i < MAX_SYNCS; i++)
// 			{
// 				if (synced_devices[i].active && strcmp(synced_devices[i].address, addr_str) == 0)
// 				{
// 					synced_devices[i].active = false;
// 					synced_devices[i].address[0] = '\0';
// 					synced_devices[i].subevent = 0;
// 					synced_devices[i].response_slot = 0;
// 					num_synced--; // Decrement active device count
// 					printk("Device disconnected and slot %d cleared\n", i);
// 				}
// 			}
// 		}
// 	}
...
// 		if (synced_devices[i].active)
// 		{
// 			printk("Device %d: subevent %d, response_slot %d\n",
// 				   i, synced_devices[i].subevent, synced_devices[i].response_slot);
// 			active_count++;
// 		}
// 	}
...
// 	for (int i = 0; i < MAX_SYNCS; i++)
// 	{
// 		if (!synced_devices[i].active && synced_devices[i].address[0] != '\0')
// 		{
...
// // Add reset_synchronization function to handle sync timeouts
...
// 		}
// 	}
// 	num_synced = 0; // Reset active device count
// 	printk("Synchronization state reset. Restarting sync...\n");
// 	// start_periodic_sync(); // Function to restart synchronization
// }

// // Add restart_advertising function to ensure sender resumes advertising
...
// 	// Initialize synced devices array
// 	for (int i = 0; i < MAX_SYNCS; i++)
// 	{
// 		synced_devices[i].active = false;
// 		synced_devices[i].subevent = 0;
...
// 	// UART is ready for polling-based input
// 	printk("UART configured for polling-based command input\n");
...
// 		// Use find_or_assign_slot to get slot/subevent for this device
...
// 		// Store synced device information
// 		synced_devices[slot_idx].active = true;
// 		synced_devices[slot_idx].subevent = subevent;
// 		synced_devices[slot_idx].response_slot = response_slot;
// 		// Address already set by find_or_assign_slot
...
// 		{
// 			printk("Write failed (err %d)\n", err);
// 			synced_devices[slot_idx].active = false;
// 			synced_devices[slot_idx].address[0] = '\0';
// 			goto disconnect;
// 		}
...
// 	printk("Command format: [+]join,<parameter>\n");
// 	printk("Example: [+]join,1234\n");

// 	// Print synced devices status
...
// // Define a delayed work item to reset the command
// static struct k_work_delayable reset_command_work;

// // Function to reset the command to default
// static void reset_command_to_default(struct k_work *work)
// {
//     strncpy(current_command, "[+]join,9999", CMD_BUF_SIZE - 1);
//     current_command[CMD_BUF_SIZE - 1] = '\0';
//     printk("Command reset to default: '%s'\n", current_command);
// }

// // Initialize the delayed work item
// void init_reset_command_work(void)
// {
//     k_work_init_delayable(&reset_command_work, reset_command_to_default);
// }

// // Modify the join command logic to schedule the reset
// static void handle_join_command(const char *cmd, const char *param)
// {
//     int join_param = atoi(param);
//     char full_cmd[64];
...
//         // Schedule the reset to default command after 7 seconds
//         k_work_schedule(&reset_command_work, K_SECONDS(7));
//     }
// }
...
#define MAX_SYNCS (NUM_SUBEVENTS * NUM_RSP_SLOTS)
#define SLOT_TIMEOUT_MS 10000 // 90 seconds
...
static const struct bt_le_per_adv_param per_adv_params = {
	.interval_min = 0x200, // 625 ms
	.interval_max = 0x300, // 937.5 ms
	.options = 0,
	.num_subevents = NUM_SUBEVENTS,
	.subevent_interval = 0x50,	   // 31.25 ms
	.response_slot_delay = 0x10,   // 6.25 ms
	.response_slot_spacing = 0x80, // 50 ms
...
static void request_cb(struct bt_le_ext_adv *adv, const struct bt_le_per_adv_data_request *request)
{
	...
	// Check if temporary command expired, and revert if needed
	update_temp_command_state();

	// Take a local snapshot of the command to send in this request
	...
	// printk("PAwR request: sent command '%s' to %u subevents (temp_active=%d)\n",
	// 	   cmd_local, to_send, temp_command_active ? 1 : 0);
}
